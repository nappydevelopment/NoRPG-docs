\section{Benutzerschnittstelle}
	...
	\subsection{Benutzeroberfläche}	
		...
		\subsubsection{Startbildschirm und Registrierung}
			...
			Auf Grund der Tatsache das es zwei Startbildschirme gibt, muss bevor der anzuzeigende Startbildschirm angezeigt wird eine Überprüfung stattfinden. Die aller erste Szene die in NoRPG geladen wird ist eine fast leere Szene, die nur aus dem Sternenhimmel, einem kleinen Textfeld und einem Skript besteht. Diese Szene ist minimal gestaltet, damit das Laden und somit das Ausführen des Skriptes sehr schnell durchgeführt werden kann, so dass der Spieler diese Szene erst gar nicht bemerkt. Das Skript prüft, ob die lokale Datei mit den Informationen über den Spielstand des Spielers vorhanden und vollständig ist. Ist dies der Fall, kann davon ausgegangen werden, dass der Spieler angemeldet ist und die zweite Variante des Startbildschirmes wird geladen. Der Sternenhimmel ist notwendig, falls die Ladezeit länger ist, damit der Spieler keinen weißen Hintergrund sieht. Das Textfeld in dieser Szene ist angebracht, damit im Fehlerfall der Spieler informiert wird.
			
			Die nächste Benutzeroberfläche die der Spieler sehen kann ist die Registrierung. Ein Teil des Registrierungsprozesses ist in Abbildung \ref{registerUI} abgebildet.
			...
			und World (Animieren der Charaktere bei Auswahl)
			
			Dialogebene = 1 (beim startbildschirm, falls passwort falsch oder so, beim registrieren beim abbrechen)
			
			Ein ganz wichtiger Punkt bei den Startbildschirmen und Registrierung ist die Konsistenz
